	...
	for (int y = 0; y < data.height; ++y)
	{
		for (int x = 0; x < data.width; ++x)
		{
			...
			if (tileId > 0)
			{
				...
				app->render->DrawTexture(tileset->texture, tileset->offsetX, tileset->offsetY);
			}
		}
	}
}
...
bool Map::LoadTilesetImage(pugi::xml_node& tileset_node, TileSet* set)
{
	...
	else
	{
		...
		/*set->numTilesWidth = image.attribute("width").as_int() / tileset_node.attribute("width").as_int();
		set->numTilesHeight = image.attribute("height").as_int() / tileset_node.attribute("height").as_int();*/
		set->offsetX = 0;
		set->offsetY = 0;
	}
...
bool Map::LoadLayer(pugi::xml_node& node, MapLayer* layer)
{
	...
	{
		tiles[x] = datanode.child("tile").attribute("gid").as_int();
		datanode.next_sibling();
	}
